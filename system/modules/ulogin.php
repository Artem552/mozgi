...
if(!defined('MOZG'))
    ...
    if(empty($user['first_name']) or empty($user['last_name']) or empty($user['identity'])) {
     ...
     $first_name = iconv("utf-8", "windows-1251", $user['first_name']);
     $last_name = iconv("utf-8", "windows-1251", $user['last_name']);
...
$subj = 'Здравствуй,пользователь!';
$message = 'Спасибо за регистрацию на сайте.
Просьба прочитать правила,баны выдаются без предупреждений.
Приглашайте друзей и получайте подарки и рейтинг. (Подробнее в настройках)
Сообщение послано автоматически,не отвечайте на него,ответа не будет! Администрация.'; // HTML разрешен
$sender_id = '2'; // ID пользователя, от которого отправляем

$db->query("INSERT INTO `".PREFIX."_messages` SET theme = '{$subj}', text = '{$message}', for_user_id = '{$id}', from_user_id = '{$sender_id}', date = '{$server_time}', pm_read = 'no', folder = 'inbox', history_user_id = '{$id}'"); 
$db->query("UPDATE `".PREFIX."_users` SET user_pm_num = user_pm_num+1 WHERE user_id = '{$id}'"); 
